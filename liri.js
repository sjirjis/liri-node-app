	...
	if (tweetCount < 20){
		client.get('search/tweets', {q: 'JirjisSteven', count: 20}, function(error, tweets, response) {
			if (error){
				...
				// console.log('');
				// console.log('Tweet # ' + (tweetCount + 1) +':');
				// console.log(tweets.statuses[tweetCount].text);
				// console.log(tweets.statuses[tweetCount].created_at);
				// console.log('----------------------------------');
...
var spotifySearch = function(spotifyQuery){
	// console.log('pre if statement', spotifyQuery);

	if (spotifyQuery === null || spotifyQuery === undefined) {
		spotifyQuery = 'The Sign';
		// console.log('inside of if', spotifyQuery);
	}
